...
<template>
  <div class="py-32 bg-primary_white">
    <div class="container">
      ...
      <div class="grid gap-5 lg:grid-cols-6" v-if="pending">
        <AnimeCardPlaceholder v-for="index in 18" :key="index" />
      </div>
      <div class="grid grid-cols-2 lg:flex lg:flex-col gap-4" v-else>
        <NuxtLink
          :to="`/anime/${anime.mal_id}`"
          v-for="(anime, index ) in animes.data.sort((a: any, b: any) => a.rank - b.rank)"
          :key="index"
        ...
        >
          <div class="flex items-center h-52 lg:h-auto justify-center w-auto gap-8">
            <h1 class="hidden lg:flex text-xl font-bold">#{{ index + 1 }}</h1>
